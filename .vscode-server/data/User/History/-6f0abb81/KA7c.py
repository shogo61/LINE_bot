...
class Database:
        ...
        try:
            with connection.cursor() as cursor:
                ...
                for i in range(len(ary1)):
                    # logging.debug(i)
                    sql = "INSERT INTO foods (name, cal) VALUES (%s,%s);"
                ...
                if self.userIn(userId) == False:
                    logging.debug('aaa')
                    sql = "INSERT INTO users (userId,cal) VALUES (%s ,0) ;"
    ...
    def get_cal(self,id_result):
        ...
        try:
            with connection.cursor() as cursor:
                sql = "SELECT cal FROM foods tWHERE id = %s;"
                cursor.execute(sql,(id_result))
    ...
    def cal_check(self,userId):
        ...
        try:
            with connection.cursor() as cursor:
                sql = "SELECT cal FROM users WHERE userId = %s;"
                cursor.execute(sql,(userId))
                connection.commit()
    ...
    def cal_in(self,cal_result,userId):
        ...
        try:
            with connection.cursor() as cursor:
                sql = "UPDATE users SET cal  = %s WHERE userId = %s;"
                logging.debug(sql)
